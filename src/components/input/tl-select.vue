<template>
  <div
    @click="focusInput"
    class="field-container relative"
    :class="{ focus: focus, error: error, success: success, disabled: disabled, 'has-value': modelValue }"
  >
    <div class="select-container relative w-full h-14 rounded-xl">
      <label v-if="label" :for="forLabel" class="select-label absolute z-10 ml-4 text-[#707070]">{{ label }}</label>
      <div class="absolute z-0 top-0 left-0 w-full">
        <select
          ref="selectElement"
          :id="forLabel"
          :value="modelValue"
          class="w-full h-full outline-0 pl-4 pr-8 pt-7"
          @focus="handleFocus"
          @blur="handleBlur"
          @change="updateValue"
        >
          <option v-if="placeholder" value="" disabled>{{ placeholder }}</option>
          <template v-if="!disabled">
            <option v-for="item in options" :key="item.value" :value="item.value">{{ item.label }}</option>
          </template>
        </select>
        <div class="absolute z-20 top-0.5 right-4 h-full flex items-center justify-center pointer-events-none">
          <ChevronDown size="22" />
        </div>
      </div>
    </div>
    <div v-if="error" class="error-container relative w-full mt-1">
      <p class="error-message text-[#df1b41] text-sm">{{ error }}</p>
    </div>
  </div>
</template>

<script>
// ICONS
import { ChevronDown } from '@lucide/vue';

export default {
  name: 'tl-select',
  components: {
    // ICONS
    ChevronDown,
  },
  props: {
    modelValue: [String, Number],
    forLabel: String,
    label: String,
    placeholder: String,
    options: Array,
    error: String,
    required: Boolean,
    disabled: {
      type: Boolean,
      default: false,
    },
    readOnly: {
      type: Boolean,
      default: false,
    },
  },
  data() {
    return {
      focus: false,
    };
  },
  methods: {
    updateValue(event) {
      if (this.disabled) return;

      this.$emit('update:modelValue', event.target.value);
    },
    handleFocus() {
      this.focus = true;
    },
    handleBlur() {
      this.focus = false;
    },
    focusInput() {
      if (!this.disabled && !this.readOnly) {
        this.$refs.selectElement.focus();
      }
    },
  },
};
</script>

<style scoped>
select {
  appearance: none !important;
  -moz-appearance: none !important;
  -webkit-appearance: none !important;
}

.field-container:not(.disabled) {
  cursor: pointer;
}

.select-container {
  background-color: rgb(245, 245, 245);

  transition-property: background-color, box-shadow;
  transition-duration: 150ms;
  transition-timing-function: ease-in-out;
}

.field-container.focus .select-container {
  box-shadow: 0 0 0 1.5px #171717 inset;
}

.field-container:not(.focus).error .select-container {
  box-shadow: 0 0 0 1.5px #df1b41 inset;
}

.field-container:not(.focus).warning .select-container {
  box-shadow: 0 0 0 1.5px #e69e2c inset;
}

.field-container:not(.focus).success .select-container {
  box-shadow: 0 0 0 1.5px #30b130 inset;
}

.field-container.disabled {
  pointer-events: none;
  opacity: 0.5;
}

.select-label {
  font-size: 1rem;
  top: 50%;
  transform: translateY(-50%);

  transition-property: font-size, transform;
  transition-duration: 150ms;
  transition-timing-function: ease-in-out;
}

.field-container.focus .select-label,
.field-container.has-value .select-label {
  font-size: 0.75rem;
  transform: translateY(-18px);
}
</style>
